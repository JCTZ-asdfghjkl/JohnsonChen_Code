...
class PrepareData:
    ...
    def load_data_per_subject(self, sub):
        ...
        music_paths = [rf'/mnt/c/Users/aardp/JohnsonChen_Code/archive/deap-dataset/audio_stimuli_MIDI/exp_id_{x}.mid' for x in range(1,41)]
        music_np_array = []
        for path in music_paths:
            x = pretty_midi.PrettyMIDI(path).get_piano_roll(fs=100)
            music_np_array.append(x)
        lens = [x.shape[-1] for x in music_np_array]
        min_len = min(lens)
        music_np_array = np.array([x[:,:min_len] for x in music_np_array]) # shape = (40, 128, min L)

        #TODO
        # take channels with non-zero std
        music_np_array = music_np_array[:,music_np_array.std(axis=(0,2))>0]
        music_np_array = music_np_array[:,23:55]
        music_np_array = music_np_array[...,:7680]

        # load average EEG features
        eeg_features = pd.read_csv('train/eeg_features_average.csv')
        #eeg_features = eeg_features.iloc[:, :4].values # shape = (40, 4)
        eeg_features = eeg_features.iloc[:,:6].values # shape = (40, 6)

        print(f'{music_np_array.shape = }')
        print(f'{eeg_features.shape = }')

        # reorder the EEG channel to build the local-global graphs
        music_np_array = self.reorder_channel(data=music_np_array, graph=self.graph_type)
        print('music_np_array:' + str(music_np_array.shape) + ' label:' + str(eeg_features.shape))
        return music_np_array, eeg_features
    ...
    def label_selection(self, label):##JOHNSON
        """
        ...
        label: (trial,4)
        """
        #if self.label_type == 'Delta':
        #    label = label[:, 0]
        #elif self.label_type == 'Theta':
        #    label = label[:, 1]
        #elif self.label_type == 'Alpha':
        #    label = label[:, 2]
        #elif self.label_type == 'Beta':
        #    label = label[:, 3]
    ...
    def split(self, data, label, segment_length, overlap, sampling_rate):
        ...
        label = np.stack([np.repeat(label[[i]], int(number_segment + 1), axis=0) for i in range(len(label))], axis=0)
                ...
                for segment in range(num_segments):
                    for channel in range(num_channels):
                        ...
                        psd, freqs = mne.time_frequency.psd_array_multitaper(
                            channel_data, sfreq, normalization='full',
                            fmin=band_range[0], fmax=band_range[1], verbose=False)
                        features[trial, segment, 0, channel, i] = 10*np.log10(np.sum(psd)*(freqs[1]-freqs[0]) + 1e-8)
