...
def train_one_epoch(data_loader, net, loss_fn, optimizer):
    ...
    for i, (x_batch, y_batch) in enumerate(data_loader):
        ...
        #_, pred = torch.max(out, 1) # only for classification
        pred = out
...
def predict(data_loader, net, loss_fn):
    ...
    with torch.no_grad():
        for i, (x_batch, y_batch) in enumerate(data_loader):
            ...
            out = net(x_batch)
            loss = loss_fn(out, y_batch)
            #_, pred = torch.max(out, 1) # for classification
            pred = out
            vl.add(loss.item())
...
def train(args, data_train, label_train, data_val, label_val, subject, fold):
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        rmse_train, mae_train, _ = get_metrics_regression(y_pred=pred_train, y_true=act_train)
        print('epoch {}, for the train set, loss={:.4f} rmse={:.4f} mae={:.4f}'
              .format(epoch, loss_train, rmse_train, mae_train))
        ...
        rmse_val, mae_val, _ = get_metrics_regression(y_pred=pred_val, y_true=act_val)
        print('epoch {}, for the validation set, loss={:.4f} rmse={:.4f} mae={:.4f}'.
              format(epoch, loss_val, rmse_val, mae_val))
        #if acc_val >= trlog['max_acc']:
        #    trlog['max_acc'] = acc_val
        #    trlog['F1'] = f1_val
        if rmse_val <= trlog['min_rmse']:
            trlog['min_rmse'] = rmse_val
            trlog['MAE'] = mae_val
        ...
        trlog['train_rmse'].append(rmse_train)
        trlog['val_loss'].append(loss_val)
        #trlog['val_acc'].append(acc_val)
        trlog['val_rmse'].append(rmse_val)
    ...
    return -trlog['min_rmse'], -trlog['MAE']
...
def test(args, data, label, reproduce, subject, fold):
    ...
    rmse, mae, _ = get_metrics_regression(y_pred=pred, y_true=act)
    print('>>> Test:  loss={:.4f} rmse={:.4f} mae={:.4f}'.format(loss, rmse, mae))
    return -rmse, pred, act
...
def combine_train(args, data_train, label_train, subject, fold, target_acc):
    ...
    #    loss_fn = LabelSmoothing(args.LS_rate)
    #else:
    #JOHNSON loss_fn = nn.CrossEntropyLoss()
    loss_fn = nn.MSELoss()
    ...
    for epoch in range(1, args.max_epoch_cmb + 1):
        ...
        rmse, mae, _ = get_metrics_regression(y_pred=pred, y_true=act)
        print('Stage 2 : epoch {}, for train set loss={:.4f} rmse={:.4f} mae={:.4f}'
              .format(epoch, loss, rmse, mae))

        if -rmse >= target_acc or epoch == args.max_epoch_cmb:
            print('early stopping!')
